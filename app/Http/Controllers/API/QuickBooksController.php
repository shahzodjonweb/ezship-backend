...
class QuickBooksController extends BaseController
{
  ...
  public function __construct()
  {
    ...
    // Log QuickBooks configuration on initialization
    Log::channel('quickbooks')->info('QuickBooks Controller initialized', [
      'sandbox' => $this->isSandbox,
      'base_url' => $this->base,
      'auth_base' => $this->authBase,
      'realm_id' => $this->realmId,
      'minor_version' => $this->minorVersion,
      'basic_token_configured' => !empty($this->basicToken)
    ]);
  }
  ...
  {
    try {
      Log::channel('quickbooks')->info('Creating QuickBooks customer', [
      ...
      if (isset($quickbooksAuth['error'])) {
        Log::channel('quickbooks')->warning('QuickBooks not configured for customer creation', [
          'error' => $quickbooksAuth['error'],
          'message' => $quickbooksAuth['message'] ?? 'Unknown error'
        ]);
      ...
      if (!$response->successful()) {
        Log::channel('quickbooks')->error('QuickBooks customer creation failed', [
      ...
      Log::channel('quickbooks')->info('QuickBooks customer created successfully', [
        'quickbooks_id' => $responseData['Customer']['Id'] ?? null,
        'display_name' => $responseData['Customer']['DisplayName'] ?? null,
      ...
      Log::channel('quickbooks')->error('Exception in createCustomer', [
  ...
  {
    try {
      Log::channel('quickbooks')->info('Creating QuickBooks invoice', [
        'load_id' => $request->id ?? null,
      ...
      if (!$currentLoad) {
        Log::channel('quickbooks')->error('Load not found for invoice creation', [
          'load_id' => $load->id
        ]);
        return [
          'status' => 'error',
      ...
      if (!$user || !$user->quickbooks_id) {
        Log::channel('quickbooks')->error('User not linked to QuickBooks', [
          'user_exists' => $user ? true : false,
          'user_id' => $user->id ?? null,
          'quickbooks_id' => $user->quickbooks_id ?? null
        ]);
        return [
          'status' => 'error',
      ...
      if (!$shipper || !$receiver) {
        Log::channel('quickbooks')->error('Invalid load locations for invoice', [
          'load_id' => $load->id,
          'has_shipper' => $shipper ? true : false,
          'has_receiver' => $receiver ? true : false
        ]);
        return [
          'status' => 'error',
      ...
      // Check if QuickBooks is configured
      if (isset($quickbooksAuth['error'])) {
        Log::channel('quickbooks')->warning('QuickBooks not configured for invoice creation', [
          'error' => $quickbooksAuth['error'],
          'message' => $quickbooksAuth['message'] ?? 'Unknown error'
        ]);
        return ['status' => 'skipped', 'message' => 'QuickBooks integration not configured'];
      ...
      if (!$response->successful()) {
        Log::channel('quickbooks')->error('QuickBooks invoice creation failed', [
          'load_id' => $load->id,
      ...
      $responseData = $response->json();
      Log::channel('quickbooks')->info('QuickBooks invoice created successfully', [
        'load_id' => $load->id,
        'invoice_id' => $responseData['Invoice']['Id'] ?? null,
        'invoice_number' => $responseData['Invoice']['DocNumber'] ?? null,
        'total_amount' => $responseData['Invoice']['TotalAmt'] ?? null,
        'response' => $responseData
      ]);
      ...
      if (isset($responseData['Invoice']['Id'])) {
        ...
        Log::channel('quickbooks')->info('Payment record created', [
          'load_id' => $load->id,
      ...
      Log::channel('quickbooks')->error('Exception in createInvoice', [
  ...
  public function sendInvoice(string $invoiceId, ?string $emailAddress = null): array
  {
    try {
      ...
      if (!$response->successful()) {
        Log::channel('quickbooks')->error('Failed to send QuickBooks invoice', [
      ...
      Log::channel('quickbooks')->info('Invoice sent successfully', [
        'invoice_id' => $invoiceId,
        'sent_to' => $sendToEmail
      ]);
      ...
      Log::channel('quickbooks')->error('Exception in sendInvoice', [
  ...
  public function updateInvoice(Request $request, string $invoiceId): array
  {
    try {
      ...
      if (!$response->successful()) {
        Log::channel('quickbooks')->error('Failed to update QuickBooks invoice', [
          'invoice_id' => $invoiceId,
          'status' => $response->status(),
          'response' => $response->json()
        ]);
      ...
      Log::channel('quickbooks')->info('Invoice updated successfully', ['invoice_id' => $invoiceId]);

      return $response->json();
    } catch (Exception $e) {
      Log::channel('quickbooks')->error('Exception in updateInvoice', [
        'invoice_id' => $invoiceId ?? null,
        'message' => $e->getMessage(),
        'trace' => $e->getTraceAsString()
      ]);
  ...
  {
    try {
      Log::channel('quickbooks')->debug('Starting token refresh process');

      $credentials = Credential::where('name', 'quickbooks')->first();

      // Check if QuickBooks credentials exist
      if (!$credentials || !$credentials->refresh_token) {
        Log::channel('quickbooks')->warning('QuickBooks credentials not found or refresh token missing', [
          'credentials_exist' => $credentials ? true : false,
          'has_refresh_token' => $credentials && $credentials->refresh_token ? true : false
        ]);
        return ['error' => 'QuickBooks not configured', 'message' => 'No valid refresh token found'];
      }

      // Check if basic token is configured
      if (empty($this->basicToken)) {
        Log::channel('quickbooks')->error('QuickBooks basic token not configured in environment');
        return ['error' => 'QuickBooks basic token not configured', 'message' => 'QUICKBOOKS_BASIC_TOKEN not set'];
      }
      ...
      if (!$response->successful()) {
        Log::channel('quickbooks')->error('Failed to refresh QuickBooks token', [
          'status' => $response->status(),
          'response' => $response->json(),
          'url' => $this->authBase . '/oauth2/v1/tokens/bearer'
        ]);
        return ['error' => 'Failed to refresh token', 'message' => 'OAuth token refresh failed'];
      }
      ...
      if (isset($responseData['access_token']) && isset($responseData['refresh_token'])) {
        ...
        Log::channel('quickbooks')->info('QuickBooks token refreshed successfully', [
          'expires_in' => $responseData['expires_in'] ?? null,
          'token_type' => $responseData['token_type'] ?? null
        ]);
      } else {
        Log::channel('quickbooks')->error('Invalid token response structure', [
          'response' => $responseData
        ]);
      }
      ...
      Log::channel('quickbooks')->error('Exception in refreshToken', [
        'message' => $e->getMessage(),
        'trace' => $e->getTraceAsString()
      ]);
      return ['error' => 'An error occurred while refreshing token', 'message' => $e->getMessage()];
    }
  }
  ...
  public function updateCustomer(Request $request, string $customerId): array
  {
    try {
      ...
      Log::channel('quickbooks')->error('Exception in updateCustomer', [
